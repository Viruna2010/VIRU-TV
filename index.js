...
app.get('/', (req, res) => res.send('Viru TV V21.0: Movie Reviews & Anime Special is LIVE! 🚀🎬'));
...
const PLAYLISTS = {
    ...
    // දවල් 2 - 3 Movie Reviews ✅ (V32 - V36)
    REVIEWS: [
        "https://github.com/Viruna2010/VIRU-TV/releases/download/v32.0/Movie_Review_1.mp4",
        "https://github.com/Viruna2010/VIRU-TV/releases/download/v33.0/Movie_Review_2.mp4",
        "https://github.com/Viruna2010/VIRU-TV/releases/download/v34.0/Movie_Review_3.mp4",
        "https://github.com/Viruna2010/VIRU-TV/releases/download/v35.0/Movie_Review_4.mp4",
        "https://github.com/Viruna2010/VIRU-TV/releases/download/v36.0/Movie_Review_5.mp4"
    ],
    // දවල් 3 - 4 Anime ✅ (V37 - V40)
    ANIME: [
...
const startEngine = () => {
    ...
    else if (hr >= 12 && hr < 14) videoToPlay = getNextVideo('COMEDY'); // 12-2 PM
    else if (hr >= 14 && hr < 15) videoToPlay = getNextVideo('REVIEWS'); // 2-3 PM
    else if (hr >= 15 && hr < 16) videoToPlay = getNextVideo('ANIME');   // 3-4 PM
...
// පැය ගානේ වීඩියෝව මාරු කිරීමට (Auto Kill)
setInterval(() => {
    const { min } = getSLTime();
    if (min === 0) { if (currentProcess) currentProcess.kill('SIGKILL'); }
}, 35000);
